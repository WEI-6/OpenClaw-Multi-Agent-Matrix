...
- `/prompts`: Contains the specialized protocols (RES, EXE, DBG, JUDGE) as `.md` templates (with `{id}` placeholders, substituted per instance).
- `/workspace`: The active `state.md` bus and execution environment (template in `state.md.example`; runtime files are not committed).
- `/workspace/agents/<id>/`: Dedicated workspace per sub-agent, with instantiated role `AGENTS.md` prompts.
- `/workspace/history`: Automatic archives of completed tasks (git-ignored).
